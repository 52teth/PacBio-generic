...
def summarize_CCS(ccs_filename='reads_of_insert.fasta', nonccs_filename='nonccs_subreads.fasta', skip_nonccs=True):
    """
    ...
    if not skip_nonccs: print("Number of non-CCS subreads: {0}".format(sum(d_nonccs.itervalues())))
    print("Avg. CCS readlength: {0}".format(sum(r_ccs.itervalues())/a))
    if not skip_nonccs: print("Avg. non-CCS subread readlength: {0}".format(sum(r_nonccs.itervalues())/len(r_nonccs)))
    
def summarize_chimera(ccs_prefix='reads_of_insert.53Aseen_trimmed_changeid.fa', nonccs_prefix='nonccs_subreads.53Aseen_trimmed_changeid.fa', skip_nonccs=True):
    """
...
def summarize_gmap(ccs_prefix='reads_of_insert.53Aseen_trimmed_changeid.fa.non_chimera.fa', nonccs_prefix='nonccs_subreads.53Aseen_trimmed_changeid.fa.non_chimera.fa', skip_nonccs=True):    
    ...
    if not skip_nonccs: nonccs_avg_obs = nonccs_obs2exp['ObsAccuracy'].mean()
    ...
    if not skip_nonccs: eval_accuracy_by_gmap.split_gmap_outcome(nonccs_prefix, nonccs_prefix+'.gff')
...
def summarize_blasr(ccs_prefix='reads_of_insert.53Aseen_trimmed_changeid.fa.non_chimera.fa',nonccs_prefix='nonccs_subreads.53Aseen_trimmed_changeid.fa.non_chimera.fa', ref_fasta_filename='/home/UNIXHOME/etseng/share/gencode/gencode.v15.pc_transcripts.non_redundant_good.fa.nonredundant.fasta', ref_size=None, skip_nonccs=True):
    """
    ...
    if not skip_nonccs: eval_refmap_by_blasr.tally_ref_hits(hit_nonccs, tally0, 0., 0.)
    eval_refmap_by_blasr.tally_ref_hits(hit_ccs, tally90, 0.9, 0.9)
    if not skip_nonccs: eval_refmap_by_blasr.tally_ref_hits(hit_nonccs, tally90, 0.9, 0.9)

    ref_count = int(os.popen("grep -c \">\" " + ref_fasta_filename).read().strip())
    if skip_nonccs: zmw_count = len(get_zmw(ccs_prefix)) 
    else: zmw_count = len(get_zmw(ccs_prefix)) + len(get_zmw(nonccs_prefix))

    a = len(tally0)
    b = ref_count
    if skip_nonccs: c = len(hit_ccs)
    else: c = len(hit_ccs) + len(hit_nonccs)
    d = zmw_count
