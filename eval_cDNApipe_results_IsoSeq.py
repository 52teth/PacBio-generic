...
def get_avg_roi_len(filename='reads_of_insert.fasta'):
    lens = np.array([len(r.seq) for r in SeqIO.parse(open(filename), 'fasta')])
    print "RoI length distribution: {0:.0f}-{1:.0f}".format(np.percentile(lens, 25), np.percentile(lens, 75))
    print "Avg. RoI length: {0:.0f}".format(np.mean(lens))

def get_flnc_len_range(filename='isoseq_flnc.fasta'):
    lens = np.array([len(r.seq) for r in SeqIO.parse(open(filename), 'fasta')])
    print "flnc length distribution: {0:.0f}-{1:.0f}".format(np.percentile(lens, 25), np.percentile(lens, 75))
...
def summarize_gmap(roi_prefix='isoseq_flnc.fasta'):
    roi_unmapped, roi_obs2exp, (roi_chimera_missed,roi_chimera_real,roi_chimera_rate), roi_avg_coverage, roi_zmw_count = eval_accuracy_by_gmap.run_gmap(roi_prefix, None, roi_prefix+'.sam')
...
def summarize_blasr(roi_prefix='isoseq_flnc.fasta', ref_fasta_filename='/home/UNIXHOME/etseng/share/gencode/gencode.v15.pc_transcripts.non_redundant_good.fa.nonredundant.fasta', ref_size=None, png_name_prefix=''):
    """
...
if __name__ == "__main__":
    ...
    # plot FL% by size bin
    cmd = "bin_FL_by_size_IsoSeq.py"
    print >> sys.stderr, "CMD:", cmd
    assert os.system(cmd) == 0
    cmd = "plot_FL_by_in.py isoseq_flnc.fasta.bin_by_len.txt {name} - purple {name}".format(name=name)
    print >> sys.stderr, "CMD:", cmd
    assert os.system(cmd) == 0

    # link over all the pngs from results/
